...
GEMBEDDING game_embedding_train_test(GRECORDS grs, PARAS myparas, char* output_file)
{
	int i, j, t;
	srand(myparas.seed);
	...
	FILE*  fp1 = fopen("test_data.txt", "w");
	fclose(fp1);
	fp1 = fopen("test_data.txt", "a");
...
int main(int argc, char* argv[])
{
	...
	GEMBEDDING ebd = game_embedding_train_test(grs, myparas, embeddingfile);
...
double matchup_fun(GEMBEDDING gebd, int a, int b, int modeltype)
{
	...
	if(gebd.d == 0) 
		...
		// printf("%d\n",a);
		// printf("%lf %lf %lf %lf\n",gebd.hvecs[a][0],gebd.hvecs[a][1],gebd.hvecs[a][2],gebd.hvecs[a][3]);
